...
def assing_driver_to_travel(driver: GET_AUTENTHICATED_DRIVER, travel: UUID) -> TravelResponseSchema:
...
def get_assigned_travels(driver: GET_AUTENTHICATED_DRIVER) -> TravelListResponseSchema:
    """ Encontra todas viagens que um motorista está escrito """
    return DriverService.find_assigned_travels(driver)

@DriverRouter.get("/")
def get_driver_info(driver: GET_AUTENTHICATED_DRIVER) -> DriverResponseSchema:
