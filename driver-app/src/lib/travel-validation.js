...
export function validateAmbulanceForTravel(ambulance) {
  if (!ambulance) {
    return {
      canAccept: false,
      message: "Você não possui uma ambulância atribuída.",
    };
  }

  if (ambulance.status === AmbulanceStatus.MAINTENANCE) {
    return {
      canAccept: false,
      message:
        "Sua ambulância está em manutenção. Aguarde a liberação pelo gerente.",
    };
  }
...
export function checkTimeConflict(novaViagem, viagensAceitas) {
  ...
  const viagensAtivas = viagensAceitas.filter(
    (v) => v.realizado === TravelStatus.EM_PROGRESSO && !v.cancelada
  );
...
export function validateTravelAcceptance(
  ...
  if (viagem.realizado === TravelStatus.EM_PROGRESSO) {
    return {
      canAccept: false,
  ...
  if (viagem.realizado === TravelStatus.REALIZADO) {
    return {
      canAccept: false,
  ...
  if (!isTravelToday(viagem.inicio) && !__DEV__) {
    return {
      canAccept: false,
